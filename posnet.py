...
class PosNet:
    ...
    def create_network(self, input_shape):
        ...
        self.feature_model = Model(resnet.input, resnet.layers[80].output)
        for l in self.feature_model.layers:
            l.trainable = False

        inp = Input(shape=input_shape)
        features = self.feature_model(inp)

        # Downsampling
        x = Conv2D(512, 3, padding='same', strides=2, kernel_regularizer=l1_l2(l1=L1_REGULARIZER, l2=L2_REGULARIZER))(features)
        ...
        x = self.residual_block(x, bottleneck_kernels=64, # OG 256
                                   out_kernels=256,       # OG 1024
                                   kernel_size=3,
                                   identity=False)
        x = Dropout(DROPOUT_RATE)(x)
        for i in range(5): # Originally 5
            x = self.residual_block(x, bottleneck_kernels=64,
                                        out_kernels=256,
                                        kernel_size=3,
                                        identity=True)
            x = Dropout(DROPOUT_RATE)(x)
