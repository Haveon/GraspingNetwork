...
def create_network(input_shape, prior, L1=0, L2=0, dropout=0, KERNEL_NUM = 16):
    ...
    # Downsampling
    x = Conv2D(8*KERNEL_NUM, 3, padding='same', strides=2, kernel_regularizer=l1_l2(l1=L1, l2=L2))(x)
    x = BatchNormalization()(x)
    x = LeakyReLU()(x)
    x = Dropout(dropout)(x)

    # Third Residual Block
    x = residual_block(x, bottleneck_kernels=4*KERNEL_NUM,
    ...
    x = residual_block(x, bottleneck_kernels=8*KERNEL_NUM,
